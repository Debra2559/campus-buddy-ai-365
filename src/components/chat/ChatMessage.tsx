...
function ChatMessageComponent({ message, previousUserContent, onToggleFavorite, userId, userAvatarUrl, userName, isStreaming = false, onSuggestedQuery }: ChatMessageProps) {
  ...
  // If a citation has no matching source, strip it out so users never see
  // dangling, non-clickable [n] tokens in the text.
    ...
    return React.Children.map(children, (child) => {
      ...
      return parts.map((part, i) => {
        ...
        if (match) {
          ...
          if (!source) {
            // No source backs this marker — hide it entirely.
            return null;
          }
          return (
